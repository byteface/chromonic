...
class LayoutBox:
    ...
    ``content_width``/``content_height`` -- the content box -- default to
    ``None`` rather than ``0.0``: leave them unset and ``getComputedStyle``
    derives the content box itself from ``width``/``height`` minus this
    element's own resolved padding and border-width (a browser reports the
    content box for the ``width``/``height`` properties regardless of
    ``box-sizing``, so a layout engine only has to report the border box it
    already computed, not split it up). Set them explicitly only if the
    engine has a cheaper or more accurate content size on hand. A percentage
    on any element resolves against its *parent's* content size the same
    way, so a layout engine only ever needs to set a box on the elements it
    actually laid out.
    """
