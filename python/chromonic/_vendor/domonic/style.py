...
class ComputedStyleDeclaration(CSSStyleDeclaration):
    ...
    def _content_size_from_box(self, box: Any, vertical: bool) -> "float | None":
        """The content-box width/height a ``LayoutBox`` implies -- what the
        ``width``/``height`` CSS properties' used value actually is (a
        browser reports the *content* box for these regardless of
        ``box-sizing``). ``box.content_width``/``content_height`` is used
        directly when the layout engine set it explicitly; otherwise it's
        derived from the border-box ``width``/``height`` a layout engine
        naturally computes, minus this element's own resolved padding and
        border-width on the relevant sides -- so a layout engine only has to
        report the border box and margins, not every box-model edge."""
        explicit = box.content_height if vertical else box.content_width
        if explicit is not None:
            return explicit
        border_box = box.height if vertical else box.width
        if vertical:
            edges = ("padding-top", "padding-bottom", "border-top-width", "border-bottom-width")
        else:
            edges = ("padding-left", "padding-right", "border-left-width", "border-right-width")
        total = 0.0
        for prop in edges:
            px = _px_value(self.getPropertyValue(prop))
            if px is None:
                return None
            total += px
        return border_box - total
